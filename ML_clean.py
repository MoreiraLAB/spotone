...
def apply_ML(input_datasets, input_methods, \
              ...
              output_header = ["Dataset","Classifier name","subset","Accuracy","AUC","Precision","Recall","F1-score"], \
  ...
  for dataset in list(input_datasets.keys()):
    ...
    for classifier_name in list(input_methods.keys()):
      classifier_pkl_name = MODELS_LOCATION + SYSTEM_SEP + \
                            INTERMEDIATE_SEP.join((dataset + INTERMEDIATE_SEP + classifier_name).split(" ")) + PKL_TERMINATION
      ...
      try:
        ...
        updated_independent_vector = [dataset, classifier_name, "Independent Test after tuning"] + updated_independent_scores
