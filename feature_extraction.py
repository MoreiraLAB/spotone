...
def location_features(input_sequence, target_residue):
    ...
    if relative_position < 0.25: 
        return [1.0]
    if (relative_position >= 0.25) and (relative_position < 0.5): 
        return [2.0]
    if (relative_position >= 0.5) and (relative_position < 0.75): 
        return [3.0]
...
def extract_amino_properties(input_res, retrieved_header=False):
    ...
    if retrieved_header == False:
        return list(res_row)[3:], list(res_row.values[0])[3:]
    elif retrieved_header == True:
        return list(res_row.values[0])[3:]
...
def extract_subsections(res_number, input_dict):
    ...
    for current_range in CENTRED_FEATURES_WINDOWS:
        if (int(res_number) - current_range) < 1:
            start_range = 1
        else:
            start_range = int(res_number) - current_range
        if (int(res_number) + current_range) > int(list(input_dict.keys())[-1]):
            end_range = int(list(input_dict.keys())[-1])
        else:
            end_range = int(res_number) + current_range
        holder_list = []
        ...
        for sequence_calc in calculated_sections:
            ...
            for amino_acid in sequence_calc:
                try:
                    ...
                    continue
            try:
                features_average = sum(holder_list) / len(holder_list)
            except:
                features_average = 0
...
def generate_simple_features(input_dict, input_file, chains_original, input_raw_file):
    ...
    for index, row in opened_file.iterrows():
        ...
        one_hot_encoding = list(
            encoding_table.loc[encoding_table[ENCODING_RESIDUE_NAME] == THREE_TO_ONE_CODE_CONVERTER[current_res]].drop(
                [ENCODING_RESIDUE_NAME], axis=1).values[0])
        relative_position = location_features(input_dict[current_chain], row[RES_NUMBER_COL])
        if (row[COMPLEX_NAME_COL] in list(discriminated_dictionary.keys())) and (
                row[CHAIN_NAME_COL] not in list(discriminated_dictionary[row[COMPLEX_NAME_COL]].keys())):
            discriminated_dictionary[row[COMPLEX_NAME_COL]][row[CHAIN_NAME_COL]] = render_discriminated_sequence_dict(
                input_dict[current_chain], row[CHAIN_NAME_COL], first_input=False)
        elif row[COMPLEX_NAME_COL] not in list(discriminated_dictionary.keys()):
            discriminated_dictionary[row[COMPLEX_NAME_COL]] = render_discriminated_sequence_dict(
                input_dict[current_chain], row[CHAIN_NAME_COL], first_input=True)
        window_centred = extract_centred(discriminated_dictionary[row[COMPLEX_NAME_COL]][row[CHAIN_NAME_COL]],
                                         str(row[RES_NUMBER_COL]), template_features_table)
        if written_header is True:
